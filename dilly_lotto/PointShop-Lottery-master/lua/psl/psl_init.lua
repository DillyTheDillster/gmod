...
psl.AllData = {
	...
	JackpotIncrease = psl.IncreasedJackpot,
...
psl.JackpotIncrease = psl.IncreasedJackpot
//the winning number
psl.Number = 0
...
psl.PointsName = "Points" --* this uses whatever you set in Point Shop's config, no need to change it here too!
...
function psl:StartUp()
	...
	self.JackpotIncrease = self.AllData.JackpotIncrease
...
function psl:Defaults()
	psl.TimeLeft = psl.DefaultTime
	psl.Jackpot = psl.StartingJackpot
	psl.JackpotIncrease = psl.JackpotIncrease
	psl.Number = 0
	psl.Tickets = {}
	psl.TicketNumber = 0
	psl.Winrars = {}
...
function psl:Drawing()
	...
	if (self.WinrarsNumber > 0) then
		...
		for k,v in pairs(player.GetAll()) do
			self.Jackpot = self.Jackpot + self.JackpotIncrease
			v:PrintMessage( HUD_PRINTTALK , "PointShop Lottery: Nobody won! The Jackpot is now "..self.Jackpot.." "..self.PointsName.."!")
...
function psl:Winrar()
	...
	for k,v in pairs(self.Winrars) do
		...
		elseif psl.Pointshop == "pointshop2" then
			v:PS2_AddStandardPoints(PlayerPayout)
		end
		for i,p in pairs(player.GetAll()) do
...
function psl:CheckNumber(ply, number)
	if (number != nil && (number >= 0 && number <= psl.MaxValue)) then
		if ( ply.PS2_Wallet and ply.PS2_Wallet.points >= psl.TicketPrice ) then
			local IsGood = true
			...
			if (IsGood) then
				if (psl.CanBuy) then
					if (ply:GetNWInt("TicketsBought") < psl.MaxTickets) then
						ply:PS2_AddStandardPoints(-psl.TicketPrice)
...
function SaveData()
	...
	if file.Exists( "psl_data.txt", "DATA" ) then
		...
		psl.AllData.JackpotIncrease = psl.JackpotIncrease
