...
function _DILLY:GetPlyData(ply, which)
	...
	if (!which) then
		return stuff[1].experience || 0, stuff[1].level || 0
	elseif (which == "exp") then
		return stuff[1].experience || 0
	elseif (which == "lvl") then
		return stuff[1].level || 0
	end
end
...
function _DILLY:GetExperienceBracket(level)
	...
	if (level >= 0 && level <= 10) then
		return tonumber(_DILLY.CONFIG.experienceRequiredPerLevel["1-10"])
	elseif (level >= 11 && level <= 20) then
		return tonumber(_DILLY.CONFIG.experienceRequiredPerLevel["11-20"])
	elseif (level >= 21 && level <= 30) then
		return tonumber(_DILLY.CONFIG.experienceRequiredPerLevel["21-30"])
	elseif (level >= 31 && level <= 40) then
		return tonumber(_DILLY.CONFIG.experienceRequiredPerLevel["31-40"])
	elseif (level >= 41 && level <= 50) then
		return tonumber(_DILLY.CONFIG.experienceRequiredPerLevel["41-50"])
	elseif (level >= 51 && level <= 60) then
		return tonumber(_DILLY.CONFIG.experienceRequiredPerLevel["51-60"])
	elseif (level >= 61 && level <= 70) then
		return tonumber(_DILLY.CONFIG.experienceRequiredPerLevel["61-70"])
	elseif (level >= 71 && level <= 80) then
		return tonumber(_DILLY.CONFIG.experienceRequiredPerLevel["71-80"])
	elseif (level >= 81 && level <= 90) then
		return tonumber(_DILLY.CONFIG.experienceRequiredPerLevel["81-90"])
	elseif (level >= 91 && level <= 100) then
		return tonumber(_DILLY.CONFIG.experienceRequiredPerLevel["91-100"])
	elseif (level >= 101) then
		return tonumber(_DILLY.CONFIG.experienceRequiredPerLevel["100+"])
	end
end
...
function plyMeta:AddExpPoints(amount)
	...
	local newExp = tonumber(currentExp + amount)
...
hook.Add("PlayerInitialSpawn", "_dilly first join add ply to db", function(ply)
	...
	ply:SetNWInt("_dillyPlyLevel", _DILLY:GetPlyData(ply, "lvl"))
	ply:SetNWInt("_dillyPlyExperience", _DILLY:GetPlyData(ply, "exp"))
	local expBracket = _DILLY:GetExperienceBracket(ply:GetLvl())
	ply:SetNWInt("_dillyPlyExpBracket", expBracket)
